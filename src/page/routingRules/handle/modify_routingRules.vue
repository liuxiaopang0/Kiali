<template>
  <div class="cv-creat-page">
    <div v-loading="start_loading" element-loading-text="数据获取中" class="creat-page">
...
export default {
  ...
  methods: {
    // 获取详情数据
    async  getProlicyDetails() {
      this.start_loading = true
      routingRulesHttp.get_routerRule_detail(this.$route.params.id).then(res => {
        if (res.status_code === 1) {
          this.form_msg = res.content ? res.content : {}
          this.form_msg.gateways = (this.form_msg.gateways).map(item => { return item.name })
          this.form_msg.items = (this.form_msg.items).map((ele, index_ele) => {
            const obj = {}
            obj.configurationValue = [0]
            if (ele.uris.length > 0) {
              obj.uris = []
              ele.uris.forEach(urisItem => {
                const urisobj = { ...urisItem }
                urisItem.requestHeaders.length > 0 ? urisobj.header = true : urisobj.header = false
                urisItem.requestParams.length > 0 ? urisobj.requestParameter = true : urisobj.requestParameter = false
                obj.uris.push(urisobj)
              })
            }
            if (ele.serviceWeights.length > 0) {
              obj.serviceWeights = []
              ele.serviceWeights.forEach((serviceWeightsItem, index) => {
                const serviceWeight = {}
                if (serviceWeightsItem.service) {
                  serviceWeight.services = serviceWeightsItem.service.service_name + ',' + serviceWeightsItem.service.service_id
                  this.changeservices(serviceWeight.services, index_ele, index)
                  this.changeserviceWeights(index_ele, index)
                }
                if (serviceWeightsItem.service_port_number && serviceWeightsItem.service_port_id) serviceWeight.service_port = serviceWeightsItem.service_port_number + ',' + serviceWeightsItem.service_port_id
                if (serviceWeightsItem.weight) {
                  obj.configuration = true
                  index > 0 ? obj.configurationValue.push(obj.configurationValue[index] + serviceWeightsItem.weight) : obj.configurationValue.push(serviceWeightsItem.weight)
                }
                obj.serviceWeights.push(serviceWeight)
              })
            }
            return obj
          })
    ...
    changeservices(services, parent, index) {
      ...
      routingRulesHttp.get_services_detail(services.split(',')[1]).then(res => {
        if (res.status_code === 1) {
          this.$set(that.form_msg.items[parent].serviceWeights[index], 'portItems', res.content.servicePortEntities.length ? res.content.servicePortEntities : [])
          console.log(that.form_msg.items[parent].serviceWeights[index], ' that.form_msg.items[parent].serviceWeights[index].portItems')
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          const data = {
            ...
            items: this.getItems(),
            uuid: this.form_msg.uuid
          }
          routingRulesHttp.put_routerRule(data).then(res => {
            this.loading = false
            if (res.status_code === 1) {
              this.$message({
                message: res.status_mes,
