// import { AppenderString, TimeInMilliseconds, TimeInSeconds } from '../types/Common';
// import {
...
//   (eventName: 'fetchError', callback: (errorMessage | null, fetchParams: FetchParams) => void): void;
//   (
...
export default class GraphDataSource {
  ...
  fetchDataForNamespaces = (restParams) => {
    ...
    governanceTopologyHttp.get_graph({
      duration: '20800s',
      graphType: 'versionedApp',
      injectServiceNodes: true,
      groupBy: 'app',
      appenders: 'deadNode, sidecarsCheck, serviceEntry, istio',
      namespaces: 'chenxi-istio'
    }).then(
      response => {
        const responseData = response
        this.graphElements = responseData && responseData.elements ? decorateGraphData(responseData.elements) : EMPTY_GRAPH_DATA
        store.commit('set_graphElements', this.graphElements)
        ...
        console.log('this.graphElements', this.graphElements)
        const decoratedGraphElements = this.graphElements
  ...
  // Getters and setters
  graphData() {
    return decorateGraphData(this.graphElements)
  }
