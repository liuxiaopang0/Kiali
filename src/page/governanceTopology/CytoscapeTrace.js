...
} from './JaegerIntegration/JaegerHelper'
import { NodeType, GraphType } from './types/Graph'

export const showTrace = (cy, graphType, trace) => {
  if (!cy) {
    return
  }
...
const showSpanSubtrace = (cy, graphType, span) => {
...
const getOutboundServiceEntry = (span, cy) => {
...
const getInboundServiceEntry = (span, cy) => {
  // see https://github.com/opentracing/specification/blob/master/semantic_conventions.md
...
const findServiceEntry = (span, cy) => {
...
const findSEHost = (hostname, cy) => {
  return cy.elements(`[${CyNode.nodeType}="${NodeType.SERVICE}"]`).filter(ele => {
    const destServices = ele.data(CyNode.destServices)
    if (destServices) {
