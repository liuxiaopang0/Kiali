...
export const hasProtocolTraffic = (protocolTraffic) => {
  return (
    protocolTraffic.rates !== undefined &&
    protocolTraffic.responses !== undefined
  )
}

export function decorateGraphData(graphData) {
  ...
  const propertiesToNumber = (object, keys) => {
  ...
  if (graphData) {
    if (graphData.nodes) {
      decoratedGraph.nodes = graphData.nodes.map((node) => {
        ...
        // We can then set the 'traffic' field undefined becauzzzse it is unused in the cy element handling.
